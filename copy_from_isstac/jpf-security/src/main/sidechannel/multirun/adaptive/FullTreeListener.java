 ...
 *
 */
public class FullTreeListener extends PropertyListenerAdapter {
	...
	public FullTreeListener(Config config, JPF jpf) {
		conf = config;
	...
	public void computeAllPartitions(String setOfPCs){
		BoolExpr hardConstraints = ctx.parseSMTLIB2String(setOfPCs, null, null, null, null);
		// TODO: parse low inputs
		BitVecExpr[] lowInputs = null;
		...
		for(Set<Integer> MSS : allMSS){
			...
			if(solver.check() != Status.SATISFIABLE){
				...
				for(int i = 0; i < lowInputs.length; ++i){
					BitVecNum bv = (BitVecNum) m.evaluate(lowInputs[i], false);
					int x = bv.getBigInteger().intValueExact();
				}
				String result = null; // createInputLine(lowInput,allTheCosts);	
				sb.append(result);
			}
			solver.pop();
		}
