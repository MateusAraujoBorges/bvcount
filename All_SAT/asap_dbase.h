...
//member access function
    ...
    void set_mem_limit(unsigned int n) {
	_mem_limit = n;
    }
//some stats
...
//lit pool naming convention is following STL Vector
    ...
    unsigned int lit_pool_size(void) { 
	return _lit_pool_finish - _lit_pool_start; 
    }
    unsigned int lit_pool_free_space(void) { 
...
//functions 
    unsigned int estimate_mem_usage (void) 
	{
	unsigned int lit_pool = sizeof(CLitPoolElement) * 
	               (lit_pool_size() + lit_pool_free_space());
	unsigned int mem_vars = sizeof(CVariable) * 
                       variables().capacity();
	unsigned int mem_cls = sizeof(CClause) * 
                      clauses().capacity();
	unsigned int mem_cls_queue = sizeof(int) * 
                            _unused_clause_idx_queue.size();
	unsigned int mem_ht_ptrs =0, mem_lit_clauses = 0;
    ...
    unsigned int mem_usage(void) {
	unsigned int lit_pool = (lit_pool_size() + lit_pool_free_space()) * sizeof(CLitPoolElement);
	unsigned int mem_vars = sizeof(CVariable) * variables().capacity();
	unsigned int mem_cls = sizeof(CClause) * clauses().capacity();
	unsigned int mem_cls_queue = sizeof(int) * _unused_clause_idx_queue.size();
	unsigned int mem_ht_ptrs = 0, mem_lit_clauses = 0;
    ...
    void detail_dump_cl(ClauseIdx cl_idx, ostream & os);
