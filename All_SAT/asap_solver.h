...
#define  SOLVER_WAIT 0
#define  SOLVER_SOLVE 1
#define  SOLVER_PREPROCESS 2
#define  SOLVER_STACKLOAD  3
...
struct CSolverParameters {
  ...
  bool  startingPointRequired;
  AssignmentStack* startingPointStack;
  Assignment* initialAssignment;
  Assignments* results;
  int solutionsRequired;
  int interfaceSize;
  int leftInterfaceSize;
  int bodySize;
  int interfaceL;
  int interfaceR;
  int interfaceDecisionLevel;
  bool fullBody; 
  bool firstRun;
  Timers* timer;
};
...
//these are for conflict analysis
    ...
    vector<vector<int> >*_usrConflictClauses;
    int _maxUsrConflictSize;

protected:
    void real_solve(AssignmentStack** stopPoint, int *solFound, int deduced);
...
//for conflict analysis
    ...
    int find_min_clause_dlevel(ClauseIdx cl);	//the min dlevel of all the assigned lits in this clause 
// for conflict analysis with multiple solutions
    bool varRightToj(int var, int j, int level);
    int findFirstRootIndex(int level);
...
//top level function
    ...
    void verify_integrity(char* msg);
...
//    ClauseIdx add_clause_with_order_adjustment(int * lits, int n_lits);
    ...
    int	solve(Assignments* results, AssignmentStack** stopPoint,int *solFound);
    ...
    int getFirstClause();
    int getFirstAddedClause ();
    int getNextClause (int cl_idx);
    void markUniPhased(vector<int> &v, int left, int right);
...
string vec_tostring(vector<Type> &vec) {
 stringstream ss;
